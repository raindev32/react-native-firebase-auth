...
import { getDataFB, getDataGoogle, registerSocialMedia } from 'services/auth/loginService'
...
const handleLogin = async (navigation, {
  ...
  if (!dataSocial.email && driver === 'facebook') return Alert.alert('Login failed', 'Make sure your privacy setting is public')
  if (driver === 'google' && !dataSocial.email) return Alert.alert('Login failed', 'Make sure your privacy setting is public')
  ...
  if (dataSocial.success && dataSocial.email) {
    ...
    if (dataLogin.success) {
      ...
      Alert.alert('Login Success', `Hi ${dataLogin.data.firstName || dataSocial.name || expoSosmed.user.givenName}!`)
